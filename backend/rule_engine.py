...
def _emit(app_state, side, switch, field_path, prev, new, expected, action_type, warning, go_no_go=""):
...
def compute_next_step(app_state, side):
    ...
    checks = {
        ...
        "aux_seq_late": lambda: bool(getattr(sw, f"int_led_{p}", False)),
        ...
        "main_contactor": lambda: bool(getattr(sw, "mb_p_pde_p" if side == "P" else "mb_s_pde_s", False)),
...
def compute_propulsion_step(app_state) -> str:
    ...
    checks = {
        "main_148vdc": lambda: bool(getattr(sw_p, "mb_p_pde_p", False) and getattr(sw_s, "mb_s_pde_s", False)),
...
def refresh_telemetry_displays(app_state):
    ...
    for side in ("P", "S"):
        ...
        emg_led = bool(getattr(sw, f"emg_led_{p}", False) or getattr(sw, "led_emergency_port" if side == "P" else "emg_led_s", False))
        scrubber = bool(getattr(sw, f"co2_scrubber_{p}", False) or getattr(sw, "co2_s" if side == "S" else "co2_scrubber_p", False))
