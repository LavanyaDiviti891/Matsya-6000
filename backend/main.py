...
# Import scenario state, reset utility, and the NEW test async runner
...
async def simulate_data():
    ...
    while True:
        ...
        for var_path, value in record.items():
            ...
            # User-controlled switches (SDW test panel, breakers, etc.) must never
            # be overwritten by recorded telemetry playback.
            if var_path.startswith("switches."):
                continue
...
async def generic_toggle(state_path: str, val: Optional[str] = Form(None)):
    ...
    if val is not None:
        ...
        print(f"[TOGGLE] Set {state_path} = {val!r}")
    else:
        current_val = getattr(obj, parts[-1])
        new_val = not current_val
        setattr(obj, parts[-1], new_val)
        print(f"[TOGGLE] Flipped {state_path}: {current_val!r} -> {new_val!r}")
...
async def start_scenario():
    ...
    if scenario_task is None or scenario_task.done():
        # Spin up the test scenario as a background tracking task
