...
descent_scenario = DescentScenario()
...
def ScenarioOverlay():
    ...
    popup_text = descent_scenario.get_popup()
...
def AppLayout(active_tab="Main"):
    ...
    # ─────────────────────────────────────────────────────────────────────────
        ...
        def ToggleBlock(label, is_on, state_key):
            ...
            # ---- 20m communication ----
            if (
                "Communication" in popup_text and
                state_key == "vhs_power_p"
            ):
                highlight = True
            ...
            # ---- SDW system ----
                ...
                "SDW" in popup_text and
                state_key in (
                    "port_side_sdw_1",
                    "starboard_side_sdw_1"
                )
            ):
                highlight = True
        ...
        man_label  = "🟡 DESCEND PHASE ACTIVE" if man_is_active else ("✅ MISSION COMPLETE" if man_is_success is True else ("❌ MISSION FAILED" if man_is_success is False else "▶  START SCENARIO"))
        man_scenario_banner = Div(
            Div(
                Span("NORMAL DESCEND SOP", style="font-weight:800; color:#facc15; font-size:13px; letter-spacing:1px;"),
...
async def simulate_data():
    ...
    while True:
        ...
        descent_scenario.update(
            s.header.depth.value,
            s.switches.state)
        
        if descent_scenario.completed:

            scenario_state.success = True
            scenario_state.result_message = (
                "Normal Descend SOP executed successfully"
            )

        elif descent_scenario.failed:

            scenario_state.success = False
            scenario_state.result_message = (
                "Incorrect SOP execution"
            )
